...
class ToursListTest extends TestCase
{
    ...
    public function test_tours_list_sorts_by_starting_date_correctly(): void
    {
        ...
        $laterTour = Tour::factory()->create([
            'travel_id' => $travel->id,
            'starting_date' => now()->addDays(7),
            'ending_date' => now()->addDays(9)
        ]);
    ...
    public function test_tours_list_filters_by_starting_date_correctly(): void
    {
        ...
        $response = $this->get($endpoint . '?dateFrom=' . now()->addDays(7));
        $response->assertJsonCount(0, 'data');
    ...
    public function test_tours_list_returns_validation_errors(): void
    {
        ...
        $response = $this->get('/api/v1/travels/' . $travel->slug . '/tours?dateFrom=fdsfdsfd');
        //$response->assertStatus(422);
        $response->assertStatus(302);

        $response = $this->get('/api/v1/travels/' . $travel->slug . '/tours?priceFrom=fdsfdsfd');
        //$response->assertStatus(422);
        $response->assertStatus(302);
    }
}
